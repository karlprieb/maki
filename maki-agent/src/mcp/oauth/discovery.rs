...
pub async fn discover_resource_metadata(
    ...
    for url in resource_metadata_urls(server_url) {
        match fetch_json::<ResourceMetadata>(client, &url).await {
            Ok(meta) => return validate_resource_metadata(meta, server_url),
            Err(e) => last_err = e,
        }
    }
    Err(OAuthError::Other(format!(
...
mod tests {
    ...
    #[test_case("https://example.com/mcp", true ; "matching_resource")]
    #[test_case("https://example.com/other", false ; "different_resource")]
    fn resource_metadata_validation(resource: &str, should_pass: bool) {
        ...
        assert_eq!(
            validate_resource_metadata(meta, "https://example.com/mcp").is_ok(),
            should_pass
        );
    }
